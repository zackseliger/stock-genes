...
class MinMaxPercentage(bt.Indicator):
    ...
    def __init__(self):
        minmax = MinMax(self.data, period=self.p.period)
...
class AbsStrengthBulls(bt.Indicator):
    lines = ('bulls',)
    params = (
        ('lookback', 6),
        ('period', 2),
    ...
    def __init__(self):
        strength = AbsoluteStrengthOscillator(self.data, lookback=self.p.lookback, period=self.p.period, smoothPeriod=self.p.smoothPeriod, movav=self.p.movav)
        self.lines.bulls = strength.bulls

class AbsStrengthBears(bt.Indicator):
    lines = ('bears',)
    params = (
        ('lookback', 6),
        ('period', 2),
    ...
    def __init__(self):
        strength = AbsoluteStrengthOscillator(self.data, lookback=self.p.lookback, period=self.p.period, smoothPeriod=self.p.smoothPeriod, movav=self.p.movav)
        self.lines.bears = strength.bears
